...
class ImprovedOffensiveAgent(ReflexCaptureAgent):
    ...
    """
    def __init__(self, index, time_for_computing=0.1, time_for_attack=100):
        super().__init__(index, time_for_computing)
        ...
        self.time_for_attack = time_for_attack
    ...
    def register_initial_state(self, game_state):
        ...
        self.patrol_positions = [
            (mid_x, self.center_position[1] - 4),
            (mid_x, self.center_position[1] - 3),
            (mid_x - 1, self.center_position[1] - 2),
    ...
    def choose_action(self, game_state):
        ...
        if self.mode == "attack":
            if self.attack_start_time is None:  # Start the timer when entering attack mode
                self.attack_start_time = game_state.data.timeleft
            elapsed_time = self.attack_start_time - game_state.data.timeleft
            if elapsed_time >= self.time_for_attack:  # Switch to defend after 10 seconds
                self.mode = "defend"
